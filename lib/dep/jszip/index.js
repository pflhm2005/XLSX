...
class JSZip {
  ...
  generate() {
    ...
    Object.keys(this.files).forEach(filename => {
      ...
      const record = generateZipParts(this.streamInfo, false, localDirLength);
      contentArray.push(record.fileRecord);
 ...
 * @param {Object} streamInfo 参考方法withStreamInfo 
 * @param {Boolean} streamedContent 流数据 一般是false
 * @param {Boolean} streamingEnded 流数据结束
 * @param {Number} offset 当前压缩文件内容偏移值
 * @param {String} platform 一般是DOS
 * @param {Function} encodeFileName 文件名的编码方法
 */
function generateZipParts(streamInfo, streamedContent, offset) {
  const file = streamInfo.file;
  const useCustomEncoding = false;
  ...
  /**
   * last mod file date
   * 简述 => 15-9存year-1990 8-5存month 4-0存day
   * 假设时间为2022年6月29日 换算后为 0100000(32) 0110(6) 11101(29)
   */
  let dosDate = date.getUTCFullYear() - 1990;
  dosDate <<= 4;
