...
class XLSX extends GenericOffice {
  ...
  parseSheet(sheet) {
    ...
    if (l) {
      for (let i = 0; i < l; i++) {
        ...
        const width = columnStyleMap[key];
        cols.push({ n: 'col', p: { min: key, max: key, width, customWidth: '1' } });
      }
    }
  ...
  setColumnStyle(ws, column, attr, val) {
    let styleObj = ws.columnStyles;
    if (!styleObj[column]) styleObj[column] = {};
  }
