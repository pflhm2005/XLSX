...
class XLSX extends GenericOffice {
  ...
  async write_zip(wb, config) {
    ...
    for (let i = 0; i < SheetNames.length; i++) {
      ...
      if (sil) {
        ...
        imageList.forEach((img, i2) => {
          let imagePath = `xl/media/image${this.imageIndex + i2}.png`;
  ...
  async parseSheet(sheet) {
    ...
    let mergeAst = null;
    if (merge.length) {
      mergeAst = { n: 'mergeCells', p: { count: merge.length }, c: merge.map(ref2 => { return { n: 'mergeCell', p: { ref: ref2 } } }) };
    }
