...
class XLSX extends GenericOffice {
  ...
  write_zip(wb, config) {
    ...
    for (let i = 0; i < SheetNames.length; i++) {
      ...
      let { ref, SheetData, mergeAst, cols, typeObj } = this.parseSheet(wb.Sheets[sheetName])
      zip.file(sheetPath, this.writeXml(generateSheetAst(ref, SheetData, mergeAst, cols, typeObj)));
    }
