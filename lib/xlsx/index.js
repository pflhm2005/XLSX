...
class XLSX extends GenericOffice {
  ...
  async parseImage(typeObj, sheet, pos, config) {
    const imageData = await this.getImageData(config.src);
    ...
    console.log(imageData.xdrCol, imageData.xdrRow);
  ...
  setImage(ws, pos, img) {
    if (!ws[pos]) {
      ...
      // 进行部分补足
      for(let i = arrayInfo.maxColumn; i <= maxColumn; i++) {
        for(let j = arrayInfo.maxRow; j <= maxColumn; j++) {
          ws[transferCellPos(j - 1, i - 1)] = { v: '' };
        }
      }
